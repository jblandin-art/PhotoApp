...
class UserPhotos extends React.Component {
  ...
  handleCommentChange = (photoId, event) => {
    ...
    if (query !== null) {
      ...
      if (query.length >= 1) {
        axios.get(`/users/mentionSearch?search=${encodeURIComponent(query)}`)
          .then((response) => {
            this.setState((prevState) => ({
  ...
  handleAddComment = (photoId, event) => {
    ...
    axios.post(`/commentsOfPhoto/${photoId}`, { comment: commentText, mentions: mentionIds })
      .then((response) => {
        this.setState((prevState) => {
                    ...
                    ? photo.comments.map((comment) => (
                        ...
                        >
                          <Typography variant="body2" style={{ fontWeight: 'bold' }}>
                            <Link to={`/users/${comment.user ? comment.user._id : comment.user_id}`}>
                              {comment.user
